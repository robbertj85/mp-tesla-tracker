...
def detect_highland(text: str) -> bool:
    return _matches(text, config.HIGHLAND_PATTERNS)


def detect_trim(text: str, allow_performance: bool = True) -> str | None:
    """Return the canonical trim, or None. Highland is appended when detected.
    ...
    for trim, patterns in config.TRIM_PATTERNS:
        ...
        if _matches(text, patterns):
            base = trim
            break
    if detect_highland(text):
        if base and base not in ("RWD", "AWD / Dual Motor"):
            return f"{base} (Highland)"
        return "Highland" if not base else f"{base} (Highland)"
    return base
