...
def _attach_wltp_estimate(out_listings: list[dict]) -> None:
    """Rate each Tesla car's ActualRange against the original WLTP for its config.

    The original (as-new) WLTP per model/variant/year comes from the curated table
    in wltp.py. Tesla's `range_km` is the per-car ActualRange (lower when degraded),
    so `rangePct` = ActualRange ÷ original WLTP is a rough battery-condition signal.
    We attach `wltpEst` (the table figure) and `rangePct` to Tesla listings only
    (Marktplaats range_km already IS the factory WLTP). Implausible values (<30 or
    >120%) are dropped as data errors. Approximate, explicitly an estimate.
    """
    for r in out_listings:
        if r.get("source") != "tesla" or not r.get("range_km"):
            continue
        est = wltp.original_wltp(r.get("model"), r.get("trim"),
                                 r.get("drivetrain"), r.get("year"))
        if not est:
            continue
